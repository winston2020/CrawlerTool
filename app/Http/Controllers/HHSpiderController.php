...
class HHSpiderController extends Controller
{
    ...
    public function title()
    {
        ...
        for ($i=1;$i<94;$i++){
            $this->url[] = 'http://www.hhmmoo.com/comic/class_2/'.$i.'.html';
        }
        $this->totalPageCount = 94;
        $client = new Client();
        ...
        $pool = new Pool($client, $requests($this->totalPageCount), [
            ...
            'fulfilled'   => function ($response, $index){
                ...
                $arr = $crawler->filter('#list > div.cComicList > li')->each(function ($node,$i) use ($http) {
                    $data['href'] = 'http://www.hhmmoo.com'.$node->filter('li > a')->attr('href');
                    $data['name'] = $node->filter('li > a')->text();
                    $data['comic_img_url'] = $node->filter('li > a > img')->attr('src');
                ...
                $bool = DB::connection('mysql_main')->table('comic')->insert($arr);
                echo $bool;
                echo '<br>';
